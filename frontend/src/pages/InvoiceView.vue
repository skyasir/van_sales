			...
			<template v-else-if="d">
				<MoneyPanel>
					<div class="flex items-start justify-between gap-3">
						<div class="min-w-0 flex-1">
							...
							<p class="money mt-1 truncate text-[15px] font-semibold text-white">
								{{ d.name }}
							</p>
						...
						<StatePill :label="d.status || '—'" :variant="statusVariant" />
					</div>
					...
					<div class="mt-4 flex justify-between">
						...
						<div class="text-right">
							...
							<p
								...
								:class="
									d.outstanding_amount > 0 ? 'text-[#FDA29B]' : 'text-[#75E0A7]'
								"
							>
				...
				<div class="van-card p-3.5">
					<div
						...
						:class="i ? 'border-t border-van-subtle' : ''"
					>
						<div class="min-w-0 flex-1">
							<p class="truncate text-[14px] font-semibold text-van-text">
								{{ item.item_name }}
							</p>
							<p class="money mt-0.5 text-[11.5px] text-van-faint">
								{{ qty(item.qty) }} {{ item.uom }} × {{ money(item.rate) }}
							</p>
						</div>
						<span class="money shrink-0 text-[14px] font-semibold text-van-text">
							{{ money(item.amount) }}
						</span>
					</div>

					<div class="mt-2 border-t border-van-subtle pt-2">
						<div class="flex justify-between">
							<span class="text-[13px] text-van-muted">Net total</span>
						...
						<div
							class="mt-2 flex items-baseline justify-between border-t border-van-subtle pt-2"
						>
							<span class="text-[15px] font-semibold text-van-text"
				...
				<template v-if="d.payments.length">
					...
					<div class="van-card p-3.5">
						...
						>
							...
							<span class="money text-[14px] font-semibold text-van-text">{{
								money(p.amount)
							}}</span>
						</div>
						<div v-if="d.change_amount > 0" class="flex justify-between py-1.5">
							<span class="text-[13px] text-van-muted">Change given</span>
							<span class="money text-[14px] font-semibold text-ok">
				...
				<div class="flex gap-2">
					<FeatherIcon name="info" class="mt-0.5 h-4 w-4 shrink-0 text-van-faint" />
					<p class="text-[11.5px] leading-[17px] text-van-faint">
